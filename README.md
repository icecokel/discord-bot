...
- 채용공고: 매일 07:00, 13:00, 19:00 KST에 공식 채용 사이트를 확인하고 새 공고만 `ADMIN_ID` 관리자 DM으로 보냅니다.
...
`X_MONITOR_ENABLED=true`로 활성화하면 창 모드 Chromium으로 `https://x.com/thsottiaux`를 매시 00분·20분·40분에 확인합니다. 발송 완료 이력에 없는 글은 첫 수집부터 알림 대상입니다. 미발송 글의 **원문 전체와 Codex app-server 한국어 번역**을 관리자 DM으로 공유하며, **KST 20:00~다음 날 07:00에는 수집만 하고 07:00 배치에서 모아서 알립니다.** 긴 모음은 여러 메시지로 나눕니다. 발송에 성공한 메시지마다 `x-profile-history.json`의 `deliveries`에 발송 시각(UTC), 수신자·채널·메시지 ID, 실제 발송 내용(원문·번역), 포함 글 ID와 전송 완료 글 ID를 저장합니다. 07:00 수집 실패 시에도 저장된 대기 글은 보내고, 발송 실패·재시작 시 미전송분은 다음 주간 배치에서 재시도합니다.

번역은 주간 발송 전에 실행하고 성공한 번역을 이력에 저장합니다. 번역 실패 글은 대기 상태로 남겨 다음 주간 배치에서 재시도하며, 다른 글은 계속 전송합니다. 원문·번역은 길어도 생략하지 않고 분할하며 전송된 부분의 위치도 저장합니다. 기존 `CODEX_BIN`, `CODEX_MODEL`, `CODEX_TIMEOUT_MS` 및 Codex 로그인 설정을 재사용합니다.
...
npm run check:x-profile -- --translate
```
