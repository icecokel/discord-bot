...
- DM의 자연어 AI 답변은 현재 Discord 메시지와 첨부 메타데이터를 Hermes 컨텍스트로 전달하고, 이미지 첨부는 안전한 임시 파일로 내려받아 참조 경로를 제공합니다.
