...
- 관리자 DM의 prefix 없는 메시지는 Hermes session으로 처리합니다.
...
관리자 DM에서 prefix 없이 보낸 메시지는 Hermes session으로 전달됩니다. 이 경로는 관리자 작업, 서버 조사, 웹 검색, 브라우저 확인, 프로젝트 파일 read-only 조회를 위한 에이전트 인터페이스입니다.
...
4. 관리자 DM의 prefix 없는 메시지는 Hermes session으로 처리합니다.
5. 그 외 메시지는 처리하지 않습니다.
...
관리자 DM은 사용자+채널 단위 Hermes session을 사용하고, 봇이 관리자 최근 대화 10턴을 함께 prompt에 포함합니다. session 호출 실패 시 Hermes oneshot으로 한 번 재시도합니다. 일반 DM의 bot-managed 압축 기억은 사용하지 않습니다.
...
| `src/core/natural-language-router.ts` | 관리자 DM Hermes session 답변 |
