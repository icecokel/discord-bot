...
이 문서는 구현 전 전환 기준 문서다. 선택한 방향은 Hermes 없이 Discord bot이 Codex app-server를 직접 호출하는 구조다. 현재 운영 봇은 아직 Codex app-server를 직접 사용하지 않는다.

## 결정 사항

- Hermes gateway와 Hermes CLI provider는 목표 구조에서 제거한다.
...
관리자 DM의 기본 AI provider를 Hermes CLI 경유가 아니라 Codex app-server 직접 호출 구조로 전환한다.
...
4. 기존 Hermes 구조는 `HermesProvider`가 `hermes -z`를 실행하고 `hermesSessionName`과 `HERMES_ADMIN_TOOLSETS`를 넘긴다.
5. 60초 안에 끝나면 상태 메시지를 최종 답변으로 수정하고, 더 길어지면 완료 후 별도 메시지로 보고한다.

Codex app-server 전환 후에도 `natural-language-router`의 Discord 응답 흐름은 최대한 유지한다.
...
운영 재시작 후 thread continuity까지 필요하면 별도 JSON store를 추가한다. 현재 Hermes session store도 메모리 기반이므로, 1차 전환에서는 메모리 기반으로 맞추는 편이 범위가 작다.
...
긱뉴스 요약/번역처럼 "Codex 전용"으로 운영할지, Gemini fallback을 허용할지는 별도 정책으로 결정한다. 기존 Hermes 정책은 fallback 없이 실패 사유를 보여주는 쪽이었다.
...
1. 서버에 Codex CLI와 인증을 준비한다.
2. 로컬에 `CodexAppServerClient` 단위 테스트를 먼저 만든다.
3. `CodexProvider`를 추가하고 `AI_PROVIDER=codex` 선택을 연결한다.
4. 관리자 명령어를 `!코덱스` 또는 기존 `!헤르메스` 호환 alias로 정리한다.
5. 자연어 관리자 DM의 provider prefix를 `[Codex]`로 표시한다.
6. 문서의 Hermes 운영 기준을 Codex app-server 기준으로 갱신한다.
7. 서버 smoke test를 통과한 뒤 `.env`를 `AI_PROVIDER=codex`로 전환하고 PM2를 재시작한다.

## 남은 결정 사항

- Codex thread id를 메모리로만 유지할지, JSON store로 영속화할지.
- 운영 디렉터리만 `CODEX_WORKDIR`로 둘지, read-only 소스 checkout을 따로 둘지.
- 긱뉴스 요약/번역을 Codex 전용으로 바꿀지, Gemini fallback을 허용할지.
- Discord 명령어 이름을 `!코덱스`로 바꿀지, `!헤르메스`를 호환 alias로 남길지.
