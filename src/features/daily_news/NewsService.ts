...
# 🚨 출력 형식 준수 (매우 중요)
반드시 아래 예시와 **똑같은 마크다운 형식**으로 작성하세요. (이모지, 볼드 처리, 줄바꿈 포함)
...
# 작성 규칙
1. **헤더 금지**: "안녕하세요" 같은 인사말 절대 금지.
2. **이모지 사용 안 함**: 제목 옆에 ⚖️, 🚨 같은 이모지를 붙이지 마세요. 깔끔하게 텍스트로만 작성하세요.
3. **링크 필수**: 각 뉴스 끝에 [기사 보기](URL) 링크를 반드시 포함하세요.`;
