...
const DEFAULT_CODEX_ADMIN_SANDBOX = "workspace-write";
const DEFAULT_CODEX_ADMIN_APPROVAL_POLICY = "never";
...
const answerWithAdminAi = async (
  ...
  const assistantLabel = getAssistantLabel(providerStatus.providerName);
  ...
  const answerPromise = (async (): Promise<string> => {
    const providerOptions =
      providerStatus.providerName === "codex"
        ? {
            codexThreadKey: `${message.author.id}:${message.channel.id}`,
            codexSearch: resolveBooleanEnv(process.env.CODEX_ADMIN_SEARCH, true),
            codexSandbox:
              process.env.CODEX_ADMIN_SANDBOX || DEFAULT_CODEX_ADMIN_SANDBOX,
            codexApprovalPolicy:
              process.env.CODEX_ADMIN_APPROVAL_POLICY ||
  ...
  if (firstOutcome.kind === "background") {
    ...
    void answerPromise
      ...
      .catch(async (error: any) => {
        console.error(
          "[NaturalLanguage] 백그라운드 관리자 AI 답변 실패:",
          error.message,
        );
        await sendLongChannelMessage(
          message,
          "관리자 AI 작업이 완료되지 못했습니다. 범위를 줄여 다시 요청해주세요.",
        );
      });
  ...
  try {
    ...
    console.error("[NaturalLanguage] 관리자 AI 답변 실패:", error.message);
...
export const handleNaturalLanguageMessage = async (
  ...
  return answerWithAdminAi(message);
};
